...
if __name__ == "__main__":
    ...
    # Enhance activities: workout_type/is_race + zone-based TSS calculations
    print("[info] Enriching activities (workout_type/is_race + zone-based TSS)...")
    for i, activity in enumerate(new_rows):
        activity_id = activity.get('id')
        sport_type = activity.get('type', '').lower()
        # Ensure workout_type present, fetch from detailed endpoint as fallback
        if 'workout_type' not in activity or activity.get('workout_type') is None:
            try:
                ...
                print(f"[warning] Failed to fetch workout_type for activity {activity_id}: {e}")

        # Derive is_race boolean: True when workout_type == 1
